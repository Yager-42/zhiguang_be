...
- **位竞价（slot auction / position auction）** — 离散商业位置（feed 置顶、search 置顶）的正向拍卖；每窗口固定 M 个位置，top M 出价占位、付第 M+1 价（GSP）。本期唯一推广机制。
- **B' 竞价链路（B' auction chain）** — 完整复用 bytedance 路径的高并发竞价链路：HTTP bid command → RocketMQ ordered command → Redis Lua 权威决策 → Kafka decision log → MySQL projection / slot allocation。_Avoid_: 只用 MySQL 排序、请求内临时竞价
- **推广（promotion）** — 创作者为获得固定商业位置而发起的竞价投放；本期等同于 fixed position auction，不再包含 paid boost / 排序加权。_Avoid_: package、product、广告商品、paid boost
- **竞价窗口（auction window）** — 某一类推广资源在给定时间窗内通过 B' 链路完成 command 收单、热排名、decision log、GSP 定价与出位分配的批次单位。_Avoid_: product auction、live session
- **位分配（slot allocation）** — 竞价窗口结算后得到的具体占位结果，描述某推广在某资源位与某时间窗中的最终展示归属。_Avoid_: winner（在多槽位场景下未必准确）、成交商品
- **付费加权（paid boost）** — 已移出本期范围的历史方案：出价影响推荐排序或粉丝触达优先级，但不产生 winner / GSP / slot allocation。本期不做。_Avoid_: 把 promotion 解释成买排序权重
...
- **接入竞价系统（硬性产品目标）** — 复用团队现成 bytedance B' 竞价链路是本期**硬性指标**，非可选项。各场景角色：固定位置推广 = 正向 B' position auction（选位+定价）；悬赏 = 逆向模式（算投标质量分，人工仍定标）。本期不接 paid boost。
