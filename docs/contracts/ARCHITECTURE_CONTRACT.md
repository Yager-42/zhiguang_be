...
| **updated** | 2026-09-03 |
| **scope** | 单实例部署的单体应用 `com.tongji`（`src/main/java/com/tongji`，469 个 Java 文件）的运行时边界、模块分层、HTTP/事件/存储契约、状态机、配置键、错误码；`db/schema.sql`、`db/cassandra/init.cql`、`src/main/resources/application.yml`、`docker-compose.yml` 承载的外部系统边界 |
...
| ID | 主题 | 决定（代码证据） | 禁止 |
|----|------|------|------|
| D1 | 进程拓扑 | **单实例部署的单体 Spring Boot 3.5.10 / Java 21 应用**（`pom.xml` parent `spring-boot-starter-parent:3.5.10`、`java.version=21`），唯一应用入口为 `ZhiGuangApplication`（`ZhiGuangApplication.java`，`@SpringBootApplication` + `@EnableScheduling`）；JVM Deadline Timer 与启动恢复均以同一时刻只有一个应用实例为前提 | 引入第二个应用/服务进程承载本仓领域逻辑；将业务应用水平扩展为多个并行实例 |
...
| D10 | 推广竞价 | 系统调度器每 30 秒检查并按 `promotion.slot-auction.window-minutes`（默认 60 分钟）自动补齐当前及下一竞价场次；用户只报名系统场次，不创建场次。窗口式 slot 竞价按**无反狙击延期的英式升价拍卖**结算（GSP 已废除）：窗口 deadline 固定为 MySQL `window_end_at`，创建后不可延长；窗口共享当前价台阶，接受出价必须 ≥ `min(currentPrice+increment, cap)`，最终赢家按终态 `currentPriceCents` 第一价格结算；命中 `capPriceCents` 时 decision Lua 立即原子进入 `AUCTION_SOLD`，不得等待 deadline。窗口事务提交后必须幂等初始化 Redis、登记 `active-streams` 并注册 JVM Deadline Timer；应用启动仅一次从 MySQL 全量读取 OPEN 窗口并恢复 timer。Timer 到期只负责触发 `close.lua`，到期裁决仍以 Redis `TIME` 为唯一权威；`NOT_DUE` 必须返回 Redis `now` 与 `deadline`，Timer 按两者差值重排。timer 注册或启动恢复任一失败都必须 fail-closed：实例进入非就绪并暂停 bprime 收单，不得静默留下仍可收单但无 timer 的 OPEN 窗口。**Elia 热路径**保持不变：保证金预授权提交后同步投影 Redis；每窗口有界 flat combine，批量 Redis Lua 是唯一热裁决权威，每批最多接受一个最高有效候选；拒绝不推进版本、不写 Stream，Redis 故障不降级 MySQL。生产终态只接受 Redis Stream `AUCTION_SOLD` / `AUCTION_NO_BID`，由 `promotion.settlement` 深模块在窗口行锁下统一推导并写入第一价格结算、钱包效果、bid/escrow 状态、allocation 与窗口终态；不存在第二条直接关窗结算路径。 | 用户创建竞价场次；反狙击延长 deadline；使用 `promotion:auction:closing` ZSET、Redis 1 秒到期扫描或 MySQL 30 秒到期关窗扫描；多应用实例各自注册 timer；网关或 Java 判定竞价接受；逐请求 FIFO executor 或逐请求 Lua；固定 batching 等待；一批接受多个中间价；GSP 多槽排名/第二价格结算；使用应用时钟裁决终场；绕过 Redis terminal decision 直接结算 |
...
| D13 | 场景开关 | 外部系统默认关闭、显式开启：`canal.enabled=false`、`recommendation.gorse.enabled=false`、`promotion.bprime.enabled=false`、`moderation.llm.enabled=false`、`counter.rebuild.enabled=false`、`feed.home.mixed-enabled=false`（`application.yml`）；接受发布或评论写流量的部署必须健康连接 Canal/Kafka，且同一 Canal destination 只能有一个实例启用 Bridge；Compose 的唯一 `app` 显式设置 `CANAL_ENABLED=true`；bprime 关闭时保证金事务在冻结资金前暂停，Redis Stream worker、Deadline Timer 与实时推送不得执行 | 在 Canal/Kafka 不健康时接受发布或评论写流量；运行额外业务应用实例；生产依赖未开启的能力 |
...
| 推广 | `promotion:auction:{{windowId}}:state` / `:ranking` / `:campaign:{cid}` / `:escrow` / `:events` / `:pub` / `:wakeup` | 全部窗口 key 使用同一 Redis Cluster hash slot；state 保存创建后不可延长的固定 deadline，并以 `winnerCommandId/winnerRequestHash/winnerAck` 保存当前赢家幂等槽；escrow hash 保存 `{campaignId}:authorizedAmount/currentHold`；ranking score=`-bidAmount`；批量决策 KEYS 为公共 6 key 后接去重 campaign key，不再使用 command bucket；`:pub` 仅发送 Stream ID 唤醒 | `PromotionAuctionRedisKeys`+Lua |
| 推广 | `promotion:auction:active-streams` | set；只登记需投影/恢复的 REDIS_STREAM 窗口，不承担 timer 或到期索引职责；仅在窗口 SETTLED 且 MySQL checkpoint 已追平后移除 | `PromotionAuctionHotStateRepository`/`PromotionRedisStreamProjector` |
| 推广 | `promotion:bprime:route:{campaignId}` | 保证金授权后写入的出价路由 JSON（owner/window/post/resource/reserve/status/endAt，其中 endAt 为固定 deadline）/ 覆盖窗口结束后的有界 TTL | `PromotionBidRouteRepository` |
...
- `BID_ACCEPTED`、`AUCTION_SOLD`（cap-hit 或到期有赢家）、`AUCTION_NO_BID`（到期无出价）写 Stream；不存在 `AUCTION_EXTENDED`。普通拒绝不写 Stream、不推进版本、不持久化 command record。`BID_ACCEPTED` 载荷携带接受后的 `winnerCampaignId/currentPriceCents/nextRequiredAmount`。批量 decision Lua 每批最多写一个新 `BID_ACCEPTED`，命中 `capPriceCents` 时在同次原子裁决中立即追加 `AUCTION_SOLD`。保证金授权以 MySQL 为事实并同步投影 Redis，不写竞价 Stream。
- Pub/Sub 仅提示窗口可能有新事件。窗口初始化登记 `promotion:auction:active-streams`；单个 Stream worker 启动时从 MySQL 恢复 OPEN 或 checkpoint 未追平的 REDIS_STREAM 窗口，再每 2 秒只遍历该 set 并从 checkpoint 继续 XRANGE；不得直接消费 Pub/Sub payload 作为事实，不得扫描 Redis 全键空间。Deadline Timer 的启动恢复与 Stream 恢复分责：Timer 仅在应用启动时一次性读取 MySQL OPEN 窗口并注册固定 deadline，不得把 `active-streams` 当作到期索引。
- MySQL checkpoint 提交后，允许裁剪 `min(checkpointVersion, currentVersion-100000)` 之前的事件；checkpoint 落后或不可用时停止裁剪。窗口结算后热键保留 24 小时；`active-streams` 仅在窗口 SETTLED 且 checkpoint 已追平后退休。
...
**窗口生命周期**（`PromotionAuctionWindowService` + `PromotionAuctionScheduler` + JVM Deadline Timer）：epoch 对齐 60 分钟窗口；30 秒 scheduler 只恒保「当前 OPEN + 下一窗口」并刷新 `promotion:allocation:active:*`，不得扫描 MySQL 到期窗口或触发关窗。窗口事务提交后先幂等初始化新的 Redis hot state、登记 `active-streams`，再按不可变 `window_end_at` 注册 timer；完成注册前不得开放该窗口收单。Timer 到点调用 `close.lua`；Lua 以 Redis `TIME` 二次裁决，只有 `now >= deadline` 才原子写 `AUCTION_SOLD` / `AUCTION_NO_BID`，`NOT_DUE` 必须携带 Redis `now/deadline` 供 Timer 按剩余时间重排。创建后 deadline 不延长；decision Lua 命中 cap 时立即原子 `AUCTION_SOLD` 并结束窗口。禁止 `promotion:auction:closing` ZSET、Redis 1 秒到期扫描及 MySQL 30 秒到期关窗扫描。

**迁移围栏与 bprime 可用性**：当 `promotion.bprime.enabled=true`，进程必须以 paused、拒绝所有 bid 与 escrow ingress 的状态启动。应用启动时先一次性读取 MySQL 全部 OPEN 窗口；对每个窗口，必须验证 Redis hot state 已存在，且 `windowEndAtEpochMs` 严格等于 MySQL `window_end_at`。缺失 state 或 deadline 不一致均视为遗留 anti-snipe extension 的不兼容活跃状态，必须抛 `PromotionAuctionUnavailableException` 使启动失败；不得重置、重建或延长既有 state。仅在全部 state 验证成功后，才可恢复 `active-streams` registry 和固定 deadline timer；且仅在全部 timer 恢复/注册成功后才切换为 accepting。初始化、恢复、后续注册，或已注册 task 的 `NOT_DUE` 重排发生不可恢复失败时，必须原子地永久切回本进程 paused/拒绝收单；当前进程不得重新开放，只能通过完整重启及上述恢复流程恢复。失败时必须保留 Redis hot state 与 `active-streams`，后者仍只在窗口 SETTLED 且 MySQL checkpoint 追平后退休。

该围栏是拒绝不兼容遗留数据的升级安全措施，不是 anti-snipe 兼容：新 consumer 不得解析、投影或以 no-op 兼容 `AUCTION_EXTENDED`，也不得恢复 `closing` ZSET 或任何 closing scan。Redis `TIME` 仍是最终关窗权威，cap-hit 仍立即 SOLD，Stream terminal settlement 语义不变。

**bprime Redis 热链路**（`promotion.bprime.enabled`）：开关关闭时保证金事务在冻结资金前返回暂停。开启后，窗口提交阶段负责 Redis hot state 初始化、`active-streams` 登记与 Deadline Timer 注册；保证金授权在低频 MySQL 事务内锁定/增加 `promotion_bid_escrow.authorized_amount` 并执行一次钱包 `hold(delta)`，同事务创建 Redis 投影对账任务，提交后同步单调投影 authorizedAmount 并写 campaign→window 路由，全部成功才报告授权可用。出价由每窗口有界 flat combiner 以 `bidAmount DESC, ingressSequence ASC` 线性化重叠请求并提交有界 batch Lua；Lua 使用 Redis TIME 并原子更新 hot state、decisionVersion、Stream 与 Pub/Sub。拒绝不推进版本、不写 Stream；命中 `capPriceCents` 时立即原子追加 `AUCTION_SOLD`，不存在反狙击延期。Redis 异常、类型错误、版本失配或提交池过载统一返回 `UNAVAILABLE/PROMOTION_AUCTION_PAUSED`，不得本地接受或回退 MySQL。高活动原生 WebSocket 继续使用窗口房间索引、每连接有界单写泵和同步最终 ACK。

**决策投影与实时通知**：单个 Stream worker 从 MySQL 恢复 OPEN 或 checkpoint 未追平窗口到 active-streams；按窗口从 checkpoint 后分页读取。事件先经带版本去重的公共 fanout，再在一个 MySQL 外层事务内完成事件结构、Stream ID/version 连续性校验、投影和 checkpoint。`BID_ACCEPTED` 只 upsert 轻量 bid 并更新 escrow currentHold。`AUCTION_SOLD` / `AUCTION_NO_BID` 是唯一生产结算权威，转换为强类型 terminal input 后调用 `com.tongji.promotion.settlement`：模块锁定窗口行，校验 terminal 与持久化 bid/active escrow，统一生成稳定 `promotion-bprime:{window}:{campaign}:{capture|release}` 引用并写钱包、bid、escrow、allocation 与 guarded SETTLED 状态。SOLD winner capture 第一价格并 release 授权余量，loser/unused escrow 全释放，allocation 从原 `window_end_at` 起持续原窗口时长；NO_BID 全释放且不写 allocation。已 SETTLED 仅在既有事实一致时幂等返回，冲突失败。checkpoint 仍由外层投影拥有，因此结算写集与 checkpoint 同次提交或回滚；allocation cache 在提交后刷新。窗口 SETTLED 且 checkpoint 追平后才可从 `active-streams` 移除，关窗方式不改变 Stream、MySQL settlement、checkpoint 或热状态退休语义。
...
| `promotion.slot-auction.*` | 槽位/保留价/60min 窗口/缓存 300s；`maintenance-delay-ms=30000` 仅驱动场次补齐与 allocation 刷新，不包含结算批次或到期关窗扫描 | `PromotionProperties` |
| `promotion.bprime.*` | enabled false、热状态 24h、命令幂等 10m/分钟桶、active-streams sweep 2s、读取批次 1000、保留事件 100000、结算回看 7d、WebSocket 出站线程与有界队列、公共增量 100–250ms 自适应/调度线程/窗口上限、英式拍卖规则 `auction-rules.*`（per-resource：incrementCents/capPriceCents；无反狙击延期配置） | `PromotionBPrimeProperties` |
