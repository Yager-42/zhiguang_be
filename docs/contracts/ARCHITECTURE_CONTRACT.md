...
**窗口生命周期**（`PromotionAuctionWindowService` + `PromotionAuctionScheduler` 3×30s fixedDelay）：epoch 对齐 60 分钟窗口；恒保「当前 OPEN + 下一窗口」；`closeDueWindows` 到期（+30s 延迟）非 bprime 直结 / bprime 追加终态决策（另有 `promotion:auction:closing` ZSET + 1s 扫描，close.lua NOT_DUE 幂等自愈）；`refreshAllocations` 周期刷 `promotion:allocation:active:*`。

**bprime Redis 热链路**（`promotion.bprime.enabled`）：窗口持久化不可变 `decision_path`；已有窗口默认为 `LEGACY_BROKER`，新窗口默认 `REDIS_STREAM`，切换期间按窗口单选路径。开关关闭时保证金事务在冻结资金前返回暂停。开启后，保证金授权在低频 MySQL 事务内锁定/增加 `promotion_bid_escrow.authorized_amount` 并执行一次钱包 `hold(delta)`，同事务创建 Redis 投影对账任务；提交后同步初始化窗口状态（含英式参数 increment/cap/extend，经 route/command 透传 Lua）、登记 active-streams、单调投影 authorizedAmount、写 campaign→window 路由，全部成功才报告授权可用。出价由有界提交线程池直接执行同槽 Lua；Lua 使用 Redis TIME，写前完成 key type 与 Stream/version 锁步校验，接受时原子更新 campaign bid、共享当前价/当前赢家、ranking、currentHold、decisionVersion，XADD `<version>-0` 决策并 PUBLISH；接受和拒绝均在同槽分钟桶紧凑保存幂等结果，默认保留 10 分钟，拒绝不推进版本。Redis 异常、类型错误、版本失配或提交池过载统一返回 `UNAVAILABLE/PROMOTION_AUCTION_PAUSED`，不得本地接受或回退 MySQL。高活动原生 WebSocket 维护房间到 SessionWriter 的直接索引；每连接由专用有界线程池执行单写泵，最终 ACK 使用有界 FIFO，公共状态只保留最新批次，关键反馈优先。慢连接关键队列满载时以 `4000/backpressure` 关闭并由客户端重连恢复。

**决策投影与实时通知**：单个 Stream worker 启动时从 MySQL 恢复 OPEN 或 checkpoint 未追平的 REDIS_STREAM 窗口到 active-streams，运行期间由 Pub/Sub 唤醒并每 2 秒遍历该集合兜底；按窗口从 MySQL checkpoint 之后分页读取，每页不超过 1000 条。窗口 SETTLED 且 checkpoint 与 Stream 追平后从集合移除并设置热状态 TTL。事件先经带版本去重的公共 fanout，再在一个 MySQL 事务内完成事件结构、Stream ID 与版本连续校验及投影，避免 checkpoint 成功后 fanout 瞬时失败导致广播永久跳过。接受 BID upsert `promotion_bid` 并更新 `promotion_bid_escrow.current_hold`，不再调用钱包 hold。`AUCTION_SOLD`/`AUCTION_NO_BID` 到达时前序 BID 已按序投影，随后按**英式第一价格**结算：`AUCTION_SOLD` 时 winner 从已授权总额 capture 终态 `winningAmount`（= 最后共享价）并 release 余量，loser release 全部授权额，写单条 allocation（slot_index=0、分配期起点 = 原 `window_end_at`）；`AUCTION_NO_BID` 时全部 release、不写 allocation；再关闭 escrow、标记 SETTLED 与推进 checkpoint。`AUCTION_EXTENDED` 只推进 checkpoint 不落 MySQL（信息事件）。最终私有出价结果由 Lua 调用返回路径直接发送，不经 fanout。公共增量按房间订阅数和待写槽占比在 100–250ms 合并；`eventVersion` 等于 Stream decisionVersion，客户端发现区间缺口或裁剪缺口时必须拉 snapshot。projection/fanout 失败不得推进 checkpoint；实时消息只作为可恢复通知层。
...
**补偿分析**（`PromotionAuctionCompensationService`）：重放预期英式第一价格计划对照实际——allocation 全缺 → `PROMOTION_ALLOCATION_REBUILD`；行数/顺序不符 → **dead**；`wallet_ledger.business_ref` 缺失 → `PROMOTION_WALLET_EFFECT_REPAIR`（12 字段 payload），存在但不符 → dead。
