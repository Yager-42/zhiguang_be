...
- **技术栈**：后端 Java 21 + Spring Boot + Spring Security + Spring AI + RAG + MyBatis + MySQL + Redis + Kafka + Caffeine + MinIO/S3 对象存储 + Canal + Elasticsearch ；前端 React + Vite
- **项目细节与亮点**：
    ...
    - **发布系统**：采用渐进式发布流程，发布的图片、视频，Markdown 文档等都存入对象存储系统，采用后端发布预签名+前端直传的形式上传，节省前后端传输资源。并接入 SiliconFlow/OpenAI-compatible 模型一键生成文章摘要。
...
This repo vendors Aegis for the local Trellis + Aegis workflow.

For Codex clone-and-use setup, run:

```powershell
powershell -ExecutionPolicy Bypass -File .codex/scripts/setup-aegis.ps1
```

Treat setup as complete only when the internal doctor check reports:

- `"ok": true`
- `"workspaceSupport": "available"`
- `"configStatus": "configured"`

After first setup, restart Codex.

More detail: `docs/aegis/README.md`
