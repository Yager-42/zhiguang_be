...
class PromotionAuctionServiceTest {
    ...
    void settlesTiedBidsWithStableIdOrdering() {
        ...
        verify(walletService).captureHoldToPlatform(42L, 100L, WalletBusinessType.PROMOTION, "promotion-settle:bid:401:capture");
        verify(walletService).captureHoldToPlatform(43L, 80L, WalletBusinessType.PROMOTION, "promotion-settle:bid:402:capture");
        verify(bidMapper).markWon(401L, 0, 100L);
    ...
    void dropsBelowReserveBidsAndAppliesReserveFloorAsClearingPrice() {
        ...
        verify(walletService).releaseHold(44L, 30L, WalletLedgerReason.PROMOTION_BID_RELEASE, WalletBusinessType.PROMOTION, "promotion-settle:bid:403:release");
        // 末位 winner 402 的下一个排名出价（30）低于保留价 → 成交价取保留价 50（<= 其出价 100，不超额）
        verify(walletService).captureHoldToPlatform(43L, 50L, WalletBusinessType.PROMOTION, "promotion-settle:bid:402:capture");
    ...
    void capturesNothingAndReleasesAllWhenEveryBidBelowReserve() {
        ...
        verify(walletService).releaseHold(42L, 30L, WalletLedgerReason.PROMOTION_BID_RELEASE, WalletBusinessType.PROMOTION, "promotion-settle:bid:401:release");
