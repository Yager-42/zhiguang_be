# promotion-auction-realtime Specification

## Purpose
Define promotion auction realtime fanout, WebSocket routing, and snapshot recovery behavior.
...
The fanout consumer SHALL consume the same decision envelope used by projection. The outer envelope event type SHALL be `AUCTION_DECISION`; `BID_ACCEPTED`, `BID_REJECTED`, `AUCTION_SOLD`, and `AUCTION_NO_BID` SHALL be distinguished by nested `decision.type`, not by separate topics or type-specific partitions.
